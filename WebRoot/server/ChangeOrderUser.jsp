...
<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <div class="modal-footer">       
    ...
    <button id="closeForm" class="btn" data-dismiss="modal" aria-hidden="true">关闭</button>
  </div>
</div>
</form>        
 <script>
...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        $('#tb_onlineorder').bootstrapTable({
            url: "<%=request.getContextPath()%>/server/getUserChangeOrder.action",         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
...
function applyChange(){
	...
	{
		$.ajax({
		       url:"<%=request.getContextPath()%>/server/ChangeOrder2",
               type:"post",
               dataType:"json",
               ...
               $("#closeForm").click();
	           var opt = {
				    url: "<%=request.getContextPath()%>/server/getUserChangeOrder.action",
				    silent: true,
				    query:{
				      type:1,
				      level:2
